...
export default function useForms() {
	...
	const register =
		(validations = {}) =>
		(ref) => {
			formsFields.current = !formsFields.current.map((r) => r.ref).includes(ref) ? [{ ref, validations }, ...formsFields.current] : formsFields.current
			return ref
		}
	...
	const getErrors = (data) => {
		return formsFields.current.reduce((errors, field) => {
			...
			return {
				[field.ref.name]: getFieldError(field.ref.value, field.validations, data),
				...errors
			}
		}, {})
	}
